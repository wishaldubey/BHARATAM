    ...
    if (multiplayerState?.joinOptions?.error) {
      setTimeout(() => {
        setMultiplayerState((prev) => ({
          ...prev,
          joinOptions: { ...prev.joinOptions, error: null },
        }));
      }, 1000);
    }
  ...
  return (
    <div className="multiplayerHome">
      {multiplayerState.connected &&
        ...
        !multiplayerState.creatingGame && (
          <div style={{ pointerEvents: "all" }}>
            {multiplayerState?.guestName && (
              <div style={{ textAlign: "center", marginBottom: "10px" }}>
                <h1>
                  {text("guestMultiplayer", {
                    name: multiplayerState.guestName,
                  })}
                </h1>
              </div>
            )}
            ...
            <button
              className="gameBtn multiplayerOptionBtn publicGame"
              onClick={() => handleAction("publicDuel")}
            >
              {text("findDuel")}
            </button>
            <br />
            ...
            <button
              className="gameBtn multiplayerOptionBtn"
              onClick={() => handleAction("createPrivateGame")}
        ...
        !multiplayerState.creatingGame && (
          ...
          >
            ...
            >
              ...
              />
              <button
                className="gameBtn goBtn"
                disabled={
                  multiplayerState?.joinOptions?.gameCode?.length !== 6 ||
                  multiplayerState?.joinOptions?.progress
                }
                style={{ width: "auto" }}
                onClick={() =>
                  handleAction(
                    "joinPrivateGame",
                    multiplayerState?.joinOptions?.gameCode
                  )
                }
              >
                {text("go")}
            ...
            <p
              style={{
                color: "red",
                visibility: multiplayerState?.joinOptions?.error
                  ? "visible"
                  : "hidden",
              }}
            >
            ...
            >
              <div className="inputContainer">
                ...
                <div className="numberInput rounds">
                  <FaArrowLeft
                    onClick={() =>
                      !(multiplayerState?.createOptions?.progress !== false) &&
                      setMultiplayerState((prev) => ({
                        ...prev,
                        createOptions: {
                          ...prev.createOptions,
                          rounds: Math.max(
                            ...
                            Math.min(
                              20,
                              Number(multiplayerState.createOptions.rounds) - 1
                            )
                          ),
                        },
                      }))
                  ...
                  <input
                    ...
                    value={multiplayerState.createOptions.rounds}
                  />
                  <FaArrowRight
                    onClick={() =>
                      !(multiplayerState?.createOptions?.progress !== false) &&
                ...
                <div className="timePerRound numberInput">
                  <FaArrowLeft
                    onClick={() =>
                      ...
                      setMultiplayerState((prev) => ({
                        ...
                        createOptions: {
                          ...prev.createOptions,
                          timePerRound: Math.max(
                            1,
                            Math.min(
                              300,
                              Number(
                                multiplayerState.createOptions.timePerRound
                              ) - 10
                            )
                          ),
                        },
                  ...
                  <input
                    ...
                    placeholder={text("timePerRoundSecs")}
                    max={300}
                    onChange={(e) =>
                      enforceMinMax(e.target, () =>
                        setMultiplayerState((prev) => ({
                ...
                <label>
                  {text("country")}: {multiplayerState?.createOptions?.location}
                </label>
                <button
                  className="goBtn"
                  onClick={() => setSelectCountryModalShown(true)}
                  disabled={multiplayerState?.createOptions?.progress !== false}
                >
                  {text("change")}
                </button>
              ...
              >
                {multiplayerState?.createOptions?.progress === false
                  ? text("go")
                  : multiplayerState?.createOptions?.progress === true
                  ? `${text("creating")}...`
      ...
      />
      <BannerText
        text={multiplayerState.error}
        shown={multiplayerState.error}
      />
      <BannerText
        text={`${text("waiting")}...`}
        shown={
      ...
      {multiplayerState.inGame &&
        ...
        !multiplayerState.gameData?.public && (
          <PlayerList
            ...
            startGameHost={() => handleAction("startGameHost")}
          />
        )}
    </div>
  );
