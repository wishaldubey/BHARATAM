...
export default function Home({ locale }) {
    ...
    if (action === "createPrivateGame") {
      if (!args[0]) {
        ...
        const maxDist =
          args[0].location === "all"
            ? 20000
            : countryMaxDists[args[0].location];

        (async () => {
          const locations = [];
          for (let i = 0; i < args[0].rounds; i++) {
            const loc = await findLatLongRandom({
          ...
          ws.send(
            JSON.stringify({
              ...
              rounds: args[0].rounds,
              timePerRound: args[0].timePerRound,
              locations,
              maxDist,
            })
          );
    ...
    ws.onmessage = (msg) => {
      ...
      if (data.type === "t") {
        ...
        if (
          ...
          (Math.abs(offset) < Math.abs(timeOffset) || !timeOffset)
        ) {
      ...
      if (data.type === "cnt") {
        ...
        setMultiplayerState((prev) => {
          if (data.state === "getready") {
            ...
            const didIguess = (data.players ?? prev.gameData?.players)?.find(
              (p) => p.id === prev.gameData?.myId
            )?.final;
            if (didIguess) {
              setMultiplayerChatEnabled(true);
            } else {
          ...
          if (
            (!prev.gameData || prev?.gameData?.state === "getready") &&
            data.state === "guess"
          ) {
            setPinPoint(null);
        ...
        const player = multiplayerState.gameData.players.find(
          (p) => p.id === id
        );
        if (player) {
  ...
  function guessMultiplayer(send) {
    ...
    ws.send(
      JSON.stringify({ type: "place", latLong: pinpointLatLong, final: true })
    );
  }
  ...
  function backBtnPressed(queueNextGame = false) {
    ...
    if (multiplayerState?.inGame) {
      ...
      (multiplayerState?.creatingGame || multiplayerState?.enteringGameCode) &&
      multiplayerState?.connected
    ) {
      setMultiplayerState((prev) => {
        return {
  ...
  const ChatboxMemo = React.useMemo(
    () => (
      <ChatBox
        ...
        onToggle={() => setMultiplayerChatOpen(!multiplayerChatOpen)}
        enabled={multiplayerChatEnabled}
    ...
    [
      ...
      ws,
      multiplayerState?.gameData?.myId,
      multiplayerState?.inGame,
    ]
  );
  ...
  return (
    <>
      ...
      <AccountModal
        shown={accountModalOpen}
        session={session}
        setAccountModalOpen={setAccountModalOpen}
      />
      <SetUsernameModal
        shown={session && session?.token?.secret && !session.token.username}
        session={session}
      />
      ...
      <main
        className={`home ${jockey.className} ${roboto.className}`}
        id="main"
      >
        <BannerText
          text={`${text("loading")}...`}
          shown={
            loading && !(multiplayerState.error || multiplayerState.connecting)
          }
        />
        <BannerText
          text={`${text("connecting")}...`}
          shown={multiplayerState.connecting && !multiplayerState.error}
        />
        ...
        <div
          style={{
            display: "flex",
            alignItems: "center",
            opacity: screen !== "singleplayer" ? 1 : 0,
          }}
          className="accountBtnContainer"
        >
          <AccountBtn
            session={session}
        ...
        />
        <div
          className={`home__content ${screen !== "home" ? "hidden" : ""} ${
            process.env.NEXT_PUBLIC_CESIUM_TOKEN ? "cesium_shown" : ""
          }`}
        >
          <div className="home__ui">
            <h1 className="home__title">BHARATAM</h1>
            <div className="home__btns">
              ...
              {/* <div className="home__squarebtns">
                ...
                >
                  <button className="home__squarebtn gameBtn">
                    <FaInstagram className="home__squarebtnicon" />
                  </button>
                </Link>
        ...
        <WelcomeModal
          shown={showWelcomeModal}
          onClose={() => setShowWelcomeModal(false)}
          openGame={() => setScreen("singleplayer")}
        />
