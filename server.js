...
if (!process.env.MONGODB) {
  console.log(
    "[MISSING-ENV WARN] MONGODB env variable not set, multi-player will not work"
      .yellow
  );
  ...
  if (mongoose.connection.readyState !== 1) {
    try {
      ...
      console.error(
        "[ERROR] Database connection failed! Multiplayer disabled!".red,
        error.message
      );
      multiplayerEnabled = false;
    }
...
if (!process.env.I18NEXT_DEFAULT_CONFIG_PATH) {
  throw new Error(
    "I18NEXT_DEFAULT_CONFIG_PATH env variable not set, please set it to the path of the i18next config file"
  );
}
if (!process.env.NEXTAUTH_SECRET) {
  console.log(
    "[MISSING-ENV WARN] NEXTAUTH_SECRET env variable not set, please set it to a random string otherwise multi-player will not work"
      .yellow
  );
  multiplayerEnabled = false;
}
if (!process.env.NEXTAUTH_URL) {
  console.log(
    "[MISSING-ENV WARN] NEXTAUTH_URL env variable not set, please set it!"
      .yellow
  );
}
...
if (!process.env.GOOGLE_CLIENT_ID) {
  console.log(
    "[MISSING-ENV WARN] GOOGLE_CLIENT_ID env variable not set, please set it for multiplayer/auth!"
      .yellow
  );
  multiplayerEnabled = false;
}
if (!process.env.GOOGLE_CLIENT_SECRET) {
  console.log(
    "[MISSING-ENV WARN] GOOGLE_CLIENT_SECRET env variable not set, please set it for multiplayer/auth!"
      .yellow
  );
  multiplayerEnabled = false;
}
if (process.env.NEXT_PUBLIC_CESIUM_TOKEN) {
  console.log(
    "[INFO] NEXT_PUBLIC_CESIUM_TOKEN env variable set, showing home animation"
      .yellow
  );
}
...
const hostname = "bharatam.vercel.app";
...
registerHandler("/memdump", "GET", (req, res, query) => {
  ...
  try {
    ...
    readFileAsync(filename).then((data) => {
      ...
      res.setHeader(
        "Content-Disposition",
        "attachment; filename=memdump.heapsnapshot"
      );
      res.end(data);
...
class Game {
  ...
  start() {
    if (
      this.state !== "waiting" ||
      this.players.size < 2 ||
      this.rounds !== this.locations.length
    ) {
      return;
    }
    this.state = "getready";
    ...
    ws.on("message", async (message) => {
      ...
      if (json.type === "verify" && !player.verified) {
        ...
        if (!json.jwt || json.jwt === "not_logged_in") {
          // guest mode
          player.username =
            "Guest #" + make6DigitCode().toString().substring(0, 4);
...
setInterval(() => {
  ...
  for (const game of games.values()) {
    ...
    if (
      ...
      playerCnt > 1 &&
      game.public &&
      game.rounds === game.locations.length
    ) {
      game.start();
      ...
      if (game.curRound <= game.rounds) {
        ...
        game.nextEvtTime =
          Date.now() +
          game.waitBetweenRounds -
          (game.curRound > game.rounds ? 5000 : 0);
        game.sendStateUpdate();
      } else {
        console.log("Game ended", game.id);
        // game over
...
setInterval(() => {
  ...
  players.forEach((player) => {
    if (currentTime - player.lastPong > 60000) {
      // 60 seconds timeout
